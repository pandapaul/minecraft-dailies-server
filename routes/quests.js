    ...
    questFetcher.fetchQuestById(questId)
            .then(function (quest) {
              if (quest && quest.id === questId) {
                ...
                return reject(new Error('Invalid quest ID'))
              }
            })
            .catch(function () {
              return reject(new Error('Invalid quest ID'))
  ...
  checkAcceptedQuestLimit(username, next)
    .then(function () {
      if (!req.questStatus) {
        ...
        throw new Error('Invalid quest status [' + req.questStatus + ']')
      }
    }).catch(next)
})
  ...
  return db.progression.countAcceptedQuests(username)
        .then(function (count) {
          if (count >= maxAcceptableQuests) {
            throw new Error('You\'ve already accepted ' + maxAcceptableQuests + ' quests.')
          }
        })
}
